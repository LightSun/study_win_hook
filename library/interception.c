...
#define _LOGE(m) fprintf(stderr, #m "\n");
...
InterceptionContext interception_create_context(void)
{
    ...
    for(i = 0; i < INTERCEPTION_MAX_DEVICE; ++i)
    {
        ...
        if (device_array[i].handle == INVALID_HANDLE_VALUE) {
            ...
            _LOGE("interception_create_context >> CreateFileA failed ");
            return 0;
        }
        ...
        {
            interception_destroy_context(device_array);
            _LOGE("interception_create_context >> CreateEventA failed ");
            return 0;
        }
        ...
        {
            interception_destroy_context(device_array);
            _LOGE("interception_create_context >> DeviceIoControl failed ");
            return 0;
        }
    }
...
int interception_receive(InterceptionContext context, InterceptionDevice device, InterceptionStroke *stroke, unsigned int nstroke)
{
    ...
    if(interception_is_keyboard(device))
    {
        ...
        DeviceIoControl(device_array[device - 1].handle, IOCTL_READ, NULL, 0,
                rawstrokes, (DWORD)nstroke * sizeof(KEYBOARD_INPUT_DATA), &strokesread, NULL);
